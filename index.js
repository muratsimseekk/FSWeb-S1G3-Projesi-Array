...
function kopyala(orijinalArr) {
  const kopyaTatlar = Array.from(orijinalArr);
...
const tatlarArr = [];
let indexNo;
function indekstekiCesitiGetir(tatlarArr, indexNo) {
  return tatlarArr[indexNo];
}
...
function ismeGoreCesitCikar(cesitler, silinecekCesit) {
  let sonuc = cesitler.indexOf(silinecekCesit);
  let cesitler2 = cesitler.splice(sonuc, 1);
  return cesitler;
}

console.log(ismeGoreCesitCikar(orijinalTatlar, "Fındık"));
/* Görev 7:
...
function ismeGoreFiltrele(tatlarDizisi, filtreDegeri) {
  const yeniDizi = [];
  let filtre = tatlarDizisi.includes(filtreDegeri);
  if (filtre == true) {
    yeniDizi.push(filtreDegeri);
  }
  return yeniDizi;
}
